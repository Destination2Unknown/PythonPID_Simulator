...
def refresh():
    ...
    if (ideadtime+itau)*3 < 600:
     rangesize = 600 
    elif (ideadtime+itau)*3 >7200:
     rangesize = 7200 
    else:
    ...
    iae=iae/len(t) #measure PID performance
    
    #Plots
    plt.figure()    
    plt.subplot(2, 1, 1) 
    plt.plot(t,CV,color="green",linewidth=3,label='CV')
    ...
    plt.subplot(2,1,2)
    plt.plot(t,pterm, color="pink", linewidth=2, label='P Term')
    plt.plot(t,iterm,color="orange",linewidth=2,label='I Term')
    plt.plot(t,dterm,color="cyan",linewidth=2,label='D Term')
